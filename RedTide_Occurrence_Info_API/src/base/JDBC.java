...
public class JDBC {
	...
	public String convertDate(String tm) {
		...
		try {
			return tm.length() == 6 ? String.format("%s-%s-01 00:00:00", tm.substring(0, 4), tm.substring(4, 6))
					: tm.length() == 8
							? String.format("%s-%s-%s 00:00:00", tm.substring(0, 4), tm.substring(4, 6),
									tm.substring(6, 8))
							: tm.length() == 12
	...
	public int dumpRedTideOccur(String filePath) {
		int r = -1;
		try {
